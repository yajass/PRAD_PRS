...
#for chr in 22;do
  ...
  #cauthor=Zeginni
    ...
    plink2_new --memory 12000 --threads 12 --bgen temp_files/temp.bgen ref-first --sample ~/athena/ukbiobank/imputed/ukbb.${chr}.sample --score big_mod_set 1 2 header-read cols=+scoresums --score-col-nums 3-${num_cols} --out big_small_scores/resall.${chr}.${low_author}

    rm temp_files/temp.bgen temp_files/all_rsid big_mod_set

    gzip big_small_scores/resall.${chr}.${low_author}.sscore
