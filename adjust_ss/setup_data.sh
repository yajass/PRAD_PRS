...
if [ ! -e geno_files/${lowauthor}.${chr}.bed ];then
  ...
  plink2_new --memory 8000 --bgen temp_files/${author}.${chr}.bgen ref-first --sample ~/athena/ukbiobank/imputed/ukbb.${chr}.sample --keep-fam helper_files/${author}_eid --maf 0.01 --geno 0.1 --hwe 1e-50 midp --make-bed --out geno_files/${lowauthor}.${chr}
